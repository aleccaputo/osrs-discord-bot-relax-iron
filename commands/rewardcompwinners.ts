...
export const command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const comp = compId
            ? await getCompParticipantsSorted(interaction.guild, compId)
            : await getRecentEndedCompetitionSortedAndGained(interaction.guild);

        console.log(comp);
        await interaction.editReply(`Calculating points for ${comp?.competition.title}`);
