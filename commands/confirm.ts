...
export const command = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption((o) => o.setName('user').setDescription('the user to confirm')),
                    ...
                    if (reportingChannel && reportingChannel.type === ChannelType.GuildText) {
                        if (e instanceof UserExistsException) {
                            await reportingChannel.send(
                                `<@${discordUser?.id}> is already a user in the system (potential server re-join). Please ensure their discord profile is set correctly.`
                            );
